...
- **2026-06-07**: 修复 Uvicorn 重载挂起问题，通过精准取消会话任务、WebSocket Sentinel 哨兵推送和设置 1 秒优雅关闭超时实现秒级干净重启
- **2026-06-07**: 修复消息窗口裁剪导致的 API 400 格式错误，将对话历史摘要置于最前端并过滤旧摘要，确保 tool calls 和 tool messages 严格相邻
- **2026-06-07**: 解决 Safari 浏览器 SVG `width=""` 警告及竞态重绘问题，引入唯一渲染 ID 并在流程图内容未改变时避免重绘，降低 CPU 消耗
- **2026-06-07**: 修复 highlight.js 在浏览器端的 `require` 未定义 ReferenceError，改为引入浏览器专属的 cdnjs 静态构建包
- **2026-06-07**: 合并工具调用与完成事件为一个在体事件、防止 agent 节点更新后 token 数量被隐藏，并对多次 agent 输出增加流式时间线分割隔离
