...
├── docs/modules/                      # 模块设计说明
├── CLAUDE.md                          # 可按 [context_tag] 分段的静态规则
├── README.md
├── run_cli.sh
└── run_web.sh
```
...
| `context_tags` | 动态上下文标签，用来懒加载 `CLAUDE.md` 片段和 Agent Notes。 |
...
| `orchestrator_think/message/prompt` | Web 调试与可观察性字段，记录编排器提示词和原始输出。 |
...
- `tests/test_dynamic_context.py`：`CLAUDE.md` 和 Agent Notes 是否按 `context_tags` 懒加载。
