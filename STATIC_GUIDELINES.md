...
# Web 前端展示规范 [web]
当你的答复中包含特定的可交互内容（如地图渲染代码）时，必须严格遵守以下格式：
1. **使用专用标识符**：必须将交互代码放在 ` ```widget ` 和 ` ``` ` 包裹的 Markdown 代码块中。
2. **绝对禁止修改标识符**：不要自作主张将 ` ```widget ` 改为 ` ```json `、` ```javascript ` 或去掉反引号。任何改动都会导致前端组件匹配失败并产生严重的展示问题。

# 结构化工具开发规范 [python]
在开发或修改本项目工具（`app/tools/`）时，如需返回同时供大模型理解（自然语言提示）和前端组件渲染（JSON/内部数据对象）的结果，应优先使用 `StructuredToolResult`：
1. 导入：`from app.tools.tool_results import StructuredToolResult`
2. 用法：返回 `StructuredToolResult(system_content="...", ui_data={...})`。这样能有效避免大量的原始数据（如经纬度列表）淹没大模型上下文窗口，同时将结构化数据安全地透传给前端 UI。
