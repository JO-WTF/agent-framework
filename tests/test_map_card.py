...
class MapCardToolTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_render_map_card_stores_and_publishes_card(self, mock_session, mock_store, mock_publish):
        ...
        self.assertIn("地图卡片已创建", result)
        mock_store.assert_called_once()
        stored_payload = json.loads(mock_store.call_args[0][1])
        self.assertEqual(stored_payload["points"][0]["label"], "站点 A")
