...
Web 对话展示使用可复用的 `conversation_turns` 结构：每一轮 turn 包含 `user`、可选 `assistant` 和 `cards` 数组。`cards` 是通用卡片容器，当前支持 `type="map"` 的地图卡片，后续图表、表格或其他可视化卡片可复用同一结构追加到同一轮对话中。一轮对话允许有多个卡片；历史兼容的 `map_cards` 仍保留为地图卡片平铺索引。
